...
namespace Ventas.Controllers.Reportes
{
    public class REPAntiguedadSaldosController : Controller
    {
        ...
        /*
        public ActionResult GenerarReporte(string fechaInicial = "", string fechaFinal = "")
        {
        ...
        public ActionResult GetCotizacion(string fechaInicial = "", string fechaFinal = "")
        {
            var item = new Reportes_BE();
            item.MTIPO = 27;
            item.FECHA_INICIAL = Convert.ToDateTime(fechaInicial);
        ...
        ");
            ...
            string file64 = Convert.ToBase64String(fileStream.ToArray());
            var file = new { File = file64, MimeType = "application/pdf", FileName = $"Reporte.pdf" };
            return Json(file);
        }
